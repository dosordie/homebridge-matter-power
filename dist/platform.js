...
export class MatterPowerPlatform {
    ...
    constructor(log, platformConfig, api) {
        ...
        if (!api.versionGreaterOrEqual?.(MIN_HOMEBRIDGE_VERSION)) {
            log.error(`homebridge-matter-power requires Homebridge ${MIN_HOMEBRIDGE_VERSION} or newer; `
                + `this bridge is running ${api.serverVersion}.`);
            return;
        }
        if (!api.isMatterAvailable?.() || !api.isMatterEnabled?.() || !api.matter) {
            log.error('Matter is not enabled for this bridge. This is a Matter-only plugin. '
    ...
    async publishBinding(binding, rawValue) {
        ...
        if (matterValue === undefined)
            return;
        const cacheKey = `${device.uuid}:${kind}`;
        if (this.lastMatterValues.get(cacheKey) === matterValue)
            return;
        try {
